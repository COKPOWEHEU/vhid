...
hiddevice_t HidOpenDevice(int vid, int pid, const wchar_t man[], const wchar_t prod[]){
  ...
  for(bus = usb_get_busses(); bus; bus = bus->next){
    for(dev = bus->devices; dev; dev = dev->next){
      ...
      if(len<0){printf("can not get string\n"); usb_close(handle); continue;}
      len = len/2 - 1;
...
char HidSendArr(hiddevice_t dev, void *buf, size_t size){
  int bytes_send;
  //ХЗ почему с раскомментированным USB_RECIP_INTERFACE не работает
  //ХЗ нужно ли использовать buf[0]
  bytes_send = usb_control_msg(dev,
                               USB_TYPE_CLASS | USB_ENDPOINT_OUT, // | USB_RECIP_INTERFACE,
                               USBRQ_HID_SET_REPORT,
                               USB_HID_REPORT_TYPE_FEATURE << 8 | ((char*)buf)[0],
                               0, buf, size, 5000);
...
char HidReceiveArr(hiddevice_t dev, void *buf, size_t size){
  int bytes_recv;
  bytes_recv = usb_control_msg(dev, USB_TYPE_CLASS | USB_ENDPOINT_IN, // | USB_RECIP_INTERFACE
                                  USBRQ_HID_GET_REPORT,
				    USB_HID_REPORT_TYPE_FEATURE << 8 | 1, 0, buf, size, 5000);
